...
import { Activity, Zap, MapPin, Clock, Users, Calendar, UserPlus } from 'lucide-react';
...
const GameCard: React.FC<GameCardProps> = ({ game, showActions = true }) => {
  ...
  const isGameToday = format(new Date(game.date), 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd');
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          ...
          {isGameToday && (
            <div className="text-xs text-orange-600 font-medium mt-1">Today!</div>
          )}
        </div>
      </div>
      ...
      {showActions && (
        <div className="pt-3 border-t border-gray-100 space-y-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span className="font-medium text-gray-700">{game.organizerName}</span>
          </div>
